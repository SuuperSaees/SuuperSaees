...
interface FileManagementHook {
  ...
  loading: boolean;
  handleFolderClick: (folderUuid: string, folderTitle: string, isOrderFolder?: boolean) => Promise<void>;
  handlePathClick: (index: number) => Promise<void>;
  initializeWithPath: (path: Array<{ title: string; uuid?: string }>) => Promise<void>;
}
...
export const useFileManagement = (
  ...
  const initializeWithPath = async (pathToInitialize: Array<{ title: string; uuid?: string }>) => {
        setPath(pathToInitialize);
        const lastFolder = pathToInitialize[pathToInitialize.length - 1];
        await handleFolderClick(lastFolder!.uuid ?? '', lastFolder!.title, true, true);
  };

  useEffect(() => {
    if (initialPath && initialPath.length > 0) {
      initializeWithPath(initialPath).catch(error => console.error('Error initializing with path:', error));
    } else {
      ...
      fetchRootLevel().catch(error => console.error('Error fetching root level:', error));
    }
  }, [clientOrganizationId, initialPath, selectedOption]);

    const handleFolderClick = async (folderUuid: string, folderTitle: string, isOrderFolder = false, noUpdatePath?: boolean) => {
        setLoading(true);
        setFiles([]); 
        setFolders([]);
        try {
  ...
  const updatePath = (folderTitle: string, folderUuid: string, isOrderFolder: boolean) => {
