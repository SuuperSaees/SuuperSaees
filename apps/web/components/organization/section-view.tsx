...
const ServiceButtonTriggers = dynamic(
  () => import("./service-button-triggers"),
  {
    loading: () => <Spinner className="w-4 h-4" />,
    ssr: false,
  },
...
function SectionView({
  ...
  const embeds = (embedsQuery.data ?? []).filter(
    (embed) => embed.location === "tab",
  );
  ...
  ).filter((tab) => creditsEnabled || tab !== "credits");
  ...
  const currentPath = useMemo(() => {
    return [{ title: clientOrganizationName, id: clientOrganizationId }];
      ...
      {Array.from(navigationOptionsMap.entries()).map(([key, option]) => (
        <TabsContent
          ...
          className={`max-h-full min-h-0 w-full border-t py-8 ${
            ["new", ...embeds.map((e) => e.id)].includes(key)
