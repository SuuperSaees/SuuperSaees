...
import CheckboxCombobox, { type CustomItemProps } from "~/components/ui/checkbox-combobox"
...
export function OrganizationsField({ control, setValue }: OrganizationsFieldProps) {
  ...
  const organizationOptions =
    clientsOrganizationsQuery.data?.map((org) => ({
      value: org.id,
      label: org.name,
  ...
  const handleOrganizationSelect = (value: string) => {
    const newOrg = clientsOrganizationsQuery.data?.find((org) => org.id === value)
    if (!newOrg) return
    ...
    setSelectedOrganizations((prev) => {
      ...
      const newOrgs = isSelected ? prev.filter((org) => org.id !== value) : [...prev, orgData]
  ...
  const CustomItem = ({ option }: CustomItemProps<SelectOption & { picture_url?: string }>) => (
    <div className="flex items-center gap-2 py-0.5">
      <div className="flex h-5 w-5 items-center justify-center rounded bg-gray-100">
        <span className="text-[10px] font-medium uppercase">{option.label.charAt(0)}</span>
      </div>
      ...
      <div className="flex h-fit min-h-9 w-full items-center gap-2 rounded-md border border-gray-200 bg-gray-50/50 px-3 py-1.5">
        ...
        >
          {selectedOrganizations?.map((org, index) => (
            ...
            >
              <div className="flex h-5 w-5 items-center justify-center rounded bg-gray-100">
                <span className="text-[10px] font-medium uppercase">{org.name.charAt(0)}</span>
              </div>
