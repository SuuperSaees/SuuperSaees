...
export default async function TrainingPage() {
  ...
  const organizationSettings = await getOrganizationSettingsByOrganizationId(agency ? agency.id : organization?.id ?? '', true, ['training_url']).catch((err) => {
    console.error(`Error client, getting organization settings: ${err}`)
    return []
  });

  const trainingUrl = organizationSettings?.find((setting) => setting.key === OrganizationSettings.KEYS.training_url)?.value;
  ...
  const urlHasUserIds = trainingUrl.includes('userIds=');
  if (urlHasUserIds && clientRoles.has(workspace.role ?? '')) {
    const userIds = new URL(trainingUrl).searchParams.get('userIds')?.split(',') ?? [];
  ...
  return (
      <div className="flex h-full border-t">
        <iframe src={trainingUrl} className="w-full h-full" />
      </div>
  );
}
